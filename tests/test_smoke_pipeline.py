...
def test_smoke_pipeline_runs_end_to_end() -> None:
    ...
    assert revisions[0]["revised_caption"] == "A man is holding an object beside a red car."
    assert revisions[0]["counterfactual_answers"][0]["generation"]["metadata"]["max_new_tokens"] == 32
    assert "support_score_norm" in revisions[0]["verified_claims"][0]
