...
def test_support_guard_drops_high_risk_object_and_attributes_keep_object() -> None:
    ...
    assert "an object" not in revised["revised_caption"]
    ...
    by_action = {item["claim"]: item for item in revised["revision_actions"]}
    assert by_action["tennis racket"]["action"] == "drop_object"
    assert by_action["red car"]["action"] == "drop_attribute"
...
def test_attribute_revision_keeps_surface_object() -> None:
    config = load_config(ROOT / "configs/smoke.yaml")
    config["generation"]["fake"]["vqa_outputs"]["original:young_boy"] = "No."
    config["generation"]["fake"]["vqa_outputs"]["counterfactual:young_boy"] = "No."
    extractor = ClaimExtractor.from_config(config, ROOT)
    record = {
        "sample_id": "smoke_1",
        "image_id": "smoke_1",
        "image_path": str(ROOT / "examples/smoke/smoke_1.jpg"),
        "prompt": config["prompts"]["caption"],
        "caption": "A young boy is standing beside a car.",
        "gt_objects": ["person", "car"],
        "latency_sec": 1.0,
    }
    ...
    assert "young boy" not in revised["revised_caption"]
    assert "boy" in revised["revised_caption"]
...
def test_support_guard_retains_object_when_vqa_no_but_local_support_is_high() -> None:
    ...
    config["clip_grounding"]["fake"]["score_norms"]["tennis racket"] = 4.0
    extractor = ClaimExtractor.from_config(config, ROOT)
    record = {
        "sample_id": "smoke_1",
        "image_id": "smoke_1",
...
def test_ceg_method_defaults_are_applied() -> None:
    config = load_config(ROOT / "configs/smoke.yaml")
    del config["ceg"]["risk_mode"]
    del config["ceg"]["object_revision"]

    method_config = config_for_method(config, "ceg")

    assert method_config["ceg"]["risk_mode"] == "targeted_vqa_support_guard"
    assert method_config["ceg"]["object_revision"] == "drop"


def test_ceg_method_defaults_do_not_override_explicit_config() -> None:
    config = load_config(ROOT / "configs/smoke.yaml")
    config["ceg"]["risk_mode"] = "targeted_vqa"
    config["ceg"]["object_revision"] = "generalize"

    method_config = config_for_method(config, "ceg")

    assert method_config["ceg"]["risk_mode"] == "targeted_vqa"
    assert method_config["ceg"]["object_revision"] == "generalize"
